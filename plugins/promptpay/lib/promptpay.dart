...
class PromptPay {
  ...
  static String generateQRData(String target, {double? amount}) {
    ...
    var checksum = _getCrc16XMODEM().convert(utf8.encode(data.join())).toRadixString(16).toUpperCase();
  ...
  static String getPromptPayQRWithNewAmount(String qrData, double amount) {
    ...
    var newQRData = data.fold("", (qrData, element) {
      if (element != null && element.typeID == amountID) {
        ...
        return qrData.toString() + amountData;
      }

      if (element != null) {
        return qrData.toString() + element.typeID + element.length.toString().padLeft(2, '0') + element.data;
      }
    ...
    return newQRData.toString() + _getCrc16XMODEM().convert(utf8.encode('$newQRData')).toRadixString(16).toUpperCase();
  }
  ...
  static bool isQRDataValid(String qrData) {
    ...
    final newChecksum = _getCrc16XMODEM().convert(utf8.encode(qrDataWithOutChecksum)).toRadixString(16).toUpperCase();
  ...
  static ParametricCrc _getCrc16XMODEM() {
    ...
    return new ParametricCrc(16, 0x1021, 0xFFFF, 0x0000, inputReflected: false, outputReflected: false);
  }
}
